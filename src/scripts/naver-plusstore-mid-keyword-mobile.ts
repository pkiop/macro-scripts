...
const clickPlusStoreMobile = async (page: Page) => {
  const selector = `a[class^="storeMoreLink-pc-module"]`;
...
const searchTargetItem: any = async (
  ...
  await page.waitForSelector(`[class*="_gnbContent_gnb_content"]`);
  await utils.sleep(utils.getRandomSecTuple(waitTimeList[4])); // 4 : 스크롤 전 대기
  for await (let i of Array(8).keys()) {
    // scroll 0 : 페이지 로딩 완료후 스크롤
    await page.mouse.wheel({ deltaY: 100 * i });
    await utils.sleep(
  ...
  if (
    await NaverMacroUtil.mobile.clickShoppingTargetItem(page, config.SEARCH_MID)
  ) {
    pageNum = 1;
    return page;
  }
...
export const process = async (localConfig: NaverShippingMidConfig) => {
  ...
  try {
    ...
    page = await clickPlusStoreMobile(page);
